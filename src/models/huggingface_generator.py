...
if not HUGGINGFACE_API_KEY:
    raise ValueError(
        " HUGGINGFACE_API_KEY is missing! Set it in your .env file or GitHub Secrets."
    )
...
def generate_image_with_huggingface(scoped_prompt: str) -> dict:
    ...
    payload = {
        ...
        "parameters": {"height": int(image_height), "width": int(image_width)},
    }
...
def run_huggingface_pipeline() -> dict:
    generate_image_enabled = (
        config.get("creative", {}).get("generate_image", {}).get("enabled", {})
    )
