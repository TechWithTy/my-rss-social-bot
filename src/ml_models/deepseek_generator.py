...
if not DEEPSEEK_API_KEY:
    raise ValueError(
        "❌ DEEPSEEK_API_KEY is missing! Set it in your .env file or GitHub Secrets."
    )
...
def send_message_to_deepseek() -> dict:
    ...
    try:
        ...
        print(
            f"🦈 DeepSeek Failed - Status: {response.status_code} | Reason: {response.reason} | Text: {response.text}"
        )
        return {
            "status": "error",
